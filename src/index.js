...
import { AUTOTRADER_CONFIG } from "./autotrader/config.js";
import { createState, logJsonl } from "./autotrader/state.js";
import { autotraderTick } from "./autotrader/tick.js";
import { acquireLock } from "./autotrader/lock.js";
...
const autotraderState = createState({ logsDir });

// Lockfile + lifecycle logging (helps diagnose unexpected SIGKILL / restarts)
const lock = acquireLock({ logsDir, name: "autotrader" });
if (!lock.ok) {
  logJsonl(autotraderState, {
  ...
  // Exit cleanly so schedulers don't stack sessions.
  process.exit(0);
}
...
let uiEnabled = Boolean(process.stdout?.isTTY);

function renderScreen(text) {
  if (!uiEnabled) {
    // In non-TTY contexts (background/scheduler), don't attempt cursor control.
    // Just write the latest screen as plain text.
    try {
      process.stdout.write(text);
    } catch (e) {
      // If stdout is broken, stop trying.
      if (String(e?.code || '').toUpperCase() === 'EPIPE' || String(e?.message || '').includes('EPIPE')) {
        uiEnabled = false;
      }
    }
    return;
  ...
  try {
    ...
    // If the output pipe breaks (EPIPE), disable UI and continue without crashing.
    if (String(e?.code || '').toUpperCase() === 'EPIPE' || String(e?.message || '').includes('EPIPE')) {
      uiEnabled = false;
      try { process.stdout.write(text); } catch {}
      return;
    }
    // ignore other render errors
  }
}
...
async function main() {
  ...
  while (true) {
    ...
    try {
      ...
      // Avoid O(n^2) sub-slicing (it was creating huge transient arrays every loop).
      const rsiSeriesAll = computeRsiSeries(closes, CONFIG.rsiPeriod);
      const rsiSeries = rsiSeriesAll.filter((x) => x !== null);
      ...
      // Autotrader tick (paper/dry/live). Disabled by default.
      if (AUTOTRADER_CONFIG.enabled) {
        const atRes = await autotraderTick({
          ...
          snapshot: {
            ...
            // For risk checks we use a coarse summary.
            orderBookSummary: {
              spread: spread ?? null
            }
          }
        ...
        if (atRes?.stop) {
          // Exit cleanly so scheduled sessions can end.
          process.exit(0);
        }
      }
